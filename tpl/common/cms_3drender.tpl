    ...
    <div class="row"> 
        <div class="col-md-12 nocsroll">
           <div class="grid simple">
			...
			<input class="trajektoria_item" value="0" type="checkbox" checked="checked">{$recordOldValues.wellName}<br>

      <div id="wykres_3dpath" style="width: 1200px;"><canvas width="1200" height="800" {*style="width: 1200px; height: 800px;"*}></canvas></div>
		...
		<!--make sure this is last-->
                ...
                <script>
                    ...
                    {foreach item="_point" from=$trajectory3DRender.1 key="_key" name="Points"}
                        mdata[0][{$_key}]=[{$_point.x},{$_point.y}, {$_point.z}];
                    {/foreach}  
                     
                    mdata_o[0]='{$recordOldValues.wellName}';
                    
                    mdata_pr[0]=[];
                    {literal}mdata_pr[0][0]={'pidbox':0,'info':'(0.0, 0.0, 0.0)'};{/literal}
                    {foreach item="_point" from=$trajectoryHaracteristicPoints key="_hkey" name="HPoints"}
                        mdata_pr[0][{$smarty.foreach.HPoints.iteration}]={literal}{{/literal}'pidbox':{$_hkey},'info':'({$_point.x},{$_point.y}, {$_point.z})'{literal}}{/literal};
                    {/foreach}    
