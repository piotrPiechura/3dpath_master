			...
			$this->errorMessageContainer->addMessage('parametrError');
                        ...
                        if (!empty($this->trajectoryTable)){
                            ...
                            foreach($this->trajectoryTable as $element){
                                ...
                                if ($element['section']=='PP'){
                                    ...
                                    $this->trajectoryHaracteristicPoints[count($this->trajectory3DRender[1])-1]['x'] = round( $this->trajectoryHaracteristicPoints[count($this->trajectory3DRender[1])-1]['x'],2);
                                    $this->trajectoryHaracteristicPoints[count($this->trajectory3DRender[1])-1]['y'] = round( $this->trajectoryHaracteristicPoints[count($this->trajectory3DRender[1])-1]['y'],2);
                                    $this->trajectoryHaracteristicPoints[count($this->trajectory3DRender[1])-1]['z'] = round( $this->trajectoryHaracteristicPoints[count($this->trajectory3DRender[1])-1]['z'],2);
                                }
                    ...
                    if ($this->record['wellType'] == '3d'){
                      ...
                      // print_r("Input points X[m], Y[m], Z[m], LP[m], alfa[st], beta[st] \n");
                        ...
                        elseif($this->record['well3DTrajectory'] == 'KONC' || empty($this->record['well3DTrajectory'])){
                            if ($np > 0){
                            ...
                            if ($np > 0){
                                $this->trajectoryTable = $algoritm->r3koncbt($np, $MX, $MY, $MZ, $MLP, $Malfa, $Mbeta, $step);
                                $this->defaultVsection = $algoritm->calculateVsAngle($this->trajectoryTable[1], $this->trajectoryTable[count($this->trajectoryTable)]);
                            }
                        }
                        //print_r($this->record);
                        ...
                        if (!empty($this->trajectoryTable)){
                            ...
                            foreach($this->trajectoryTable as $key => $element){
                                ...
                                if ($element['section']=='PP'){
                                    $this->trajectoryHaracteristicPoints[count($this->trajectory3DRender[1])-1] = $tmpArr;
                                    $this->trajectoryHaracteristicPoints[count($this->trajectory3DRender[1])-1]['x'] = round( $this->trajectoryHaracteristicPoints[count($this->trajectory3DRender[1])-1]['z'],2);
                                    $this->trajectoryHaracteristicPoints[count($this->trajectory3DRender[1])-1]['y'] = round( $this->trajectoryHaracteristicPoints[count($this->trajectory3DRender[1])-1]['y'],2);
                                    $this->trajectoryHaracteristicPoints[count($this->trajectory3DRender[1])-1]['z'] = round( $this->trajectoryHaracteristicPoints[count($this->trajectory3DRender[1])-1]['z'],2);
                                }
                                elseif ($element['section'] != $section){
