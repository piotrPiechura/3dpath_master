    ...
    public function inputKBT($x1, $y1, $z1, $x2, $y2, $z2, $alfa2, $beta2, $lp)
{
	...
	if ($IKS>0) {
			...
			$a=$IKS-$R;
			$b=$IGREK;
	 		if ($a>=0 && $b>=0 ) { $delta=pi() - $delta0; }
			if ($a>=0 && $b<0 ) { $delta= pi() + $delta0; }
			if ($a<0 && $b>=0 ) { $delta = $delta0; }
			if ($a<0 && $b<0 ) { $delta =2* pi() - $delta0; }
         	}
	elseif ( $IKS <= 0 ) 
		{
			$wsk= -1;	
    ...
    function coordR3k($sx, $sy, $sz, $tx, $ty, $tz, $wx, $wy, $wz, $x1 ,$y1, $z1, $xprim, $yprim, $xHPprim, $yHPprim, $x2prim, $y2prim, $r, $l, $ll, $lk, $lp){
	    ...
	    $alfa[$l] = /*rad2deg(*/ acos($kz/sqrt(pow($kx,2)+pow($ky,2)+pow($kz,2)));// ); warunek wynikaj�cy z p�aszczyzny pionowej		                
...
*/	
	    $Malfa[$i-1] = rad2deg( acos($kz/sqrt(pow($kx,2)+pow($ky,2)+pow($kz,2))) );// warunek wynikaj�cy z p�aszczyzny pionowej		                
/*	
            if ( $kx == 0 && $ky == 0) {
					$kx =  $inputK['sx']; 
					$ky =  $inputK['sy']; }
...
*/        
        ...
        function r3koncbt($np, $MX, $MY, $MZ, $MLP, $Malfa, $Mbeta, $step, $vsAngle = null){
        $step = 10;
        $LL = 0;
        $SumaLL = 0;
        $korekta = 0;
